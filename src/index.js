...
function checksExistsUserAccount(request, response, next) {
  ...
  if (!usersExists) {
    return response.status(404).json({ error: `Users doesn't exist` });
  }

  request.user = usersExists;
  return next();
}
...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  if (user.pro !== true && todosLength >= 10) {
    return response.status(403).json({ error: "User reached the todos limit" });
  }
...
function checksTodoExists(request, response, next) {
  ...
  const todoExists = userExists
    ? userExists.todos.find((todo) => todo.id === id)
    : false;
